...
public class Experiment {
    ...
    public Experiment (String expName, int runs, double initMeanGene1, double initMeanGene2, double initMeanGene3, double initVariance, double scarcity, double num_of_gens, double daphPopSize,
                       double symbPopSize, double mut_chance, double mutStepSize, String variablePar1, double varParValue1, String variablePar2, double varParValue2, String mode,
        ...
        dummyVars.getVarDict().put("initMeanGene3", initMeanGene3);
        ...
        HashMap<Double, ArrayList<Double>> susceptChanceList = new HashMap<>();
        HashMap<Double, ArrayList<Double>> attachChanceList = new HashMap<>();
        HashMap<Double, ArrayList<Double>> fitDaphsList = new HashMap<>();
        HashMap<Double, ArrayList<Double>> mutation_chance = new HashMap<>();
        HashMap<Double, ArrayList<Double>> mutStepSizeList = new HashMap<>();
        HashMap<Double, ArrayList<Double>> fitSymbsList = new HashMap<>();
        HashMap<Double, ArrayList<Double>> hostAdvList = new HashMap<>();
        ...
        bigData.getColumns().put("susceptChance", susceptChanceList);
        bigData.getColumns().put("attachChance", attachChanceList);
        bigData.getColumns().put("fitDaph", fitDaphsList);
        bigData.getColumns().put("mut_chance", mutation_chance);
        bigData.getColumns().put("mutStepSize", mutStepSizeList);
        bigData.getColumns().put("fitSymb", fitSymbsList);
        bigData.getColumns().put("hostAdv", hostAdvList);
        bigData.getColumns().put("symbAdv", symbSlopes);
    ...
    public int startExp() {
        ...
        for (int runNum = 0; runNum < runs; runNum++) {
            ...
            bigData = new Simulation().simulator(varDict, bigData, datapoints, variablePar1, variablePar2, mode, modeArgs);
        }

        MeanData maeniee = new MeanData().calcMeansVariance(bigData);
        try {
            new Simulation().toTXT(bigData, maeniee, varDict, mode, variablePar1, varParValue1, variablePar2, varParValue2, expName);
